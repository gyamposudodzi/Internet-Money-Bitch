...
create index if not exists idx_movies_publication_status on public.movies(publication_status, published_at desc);
create index if not exists idx_series_publication_status on public.series(publication_status, published_at desc);
create index if not exists idx_episodes_publication_status on public.episodes(publication_status, published_at desc);
create index if not exists idx_audio_publication_status on public.audio_items(publication_status, published_at desc);
create index if not exists idx_content_files_lookup on public.content_files(content_kind, content_id, is_active);
create index if not exists idx_download_sessions_user_id on public.download_sessions(user_id, created_at desc);
create index if not exists idx_download_sessions_token on public.download_sessions(session_token);
create index if not exists idx_ad_events_user_id on public.ad_events(user_id, created_at desc);
...
-- Recommended next step after applying this schema:
-- 1. Enable RLS on user-facing tables.
-- 2. Add policies for public catalog read access and admin-only writes.
-- 3. Add trigger functions for updated_at and points ledger consistency.
